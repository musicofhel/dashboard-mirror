...
A standalone tool that captures a complete mirror of OpenObserve dashboards (screenshots, DOM, API, config diffs, alerts, traces, functions, health) and runs multi-agent analysis to produce grounding documents — canonical references for what each dashboard looks like.
...
src/dashboard_mirror/
  api.py             — Shared OO API client (GET, POST, v2, noauth, root)
  ...
  alerts.py          — Alert rules, history, incidents, drift detection
  functions.py       — VRL functions & pipeline capture
  traces.py          — Deep trace structure analysis
  health.py          — OO instance health & config
  supplementary.py   — Saved views, reports, annotations, folders
  collect_all.py     — Orchestrator for all collectors
